const Banner = () => {
  return (
    <div className="min-h-screen flex justify-center items-center">
      <div className="w-3/4 mx-auto flex flex-col gap-9 text-center">
        <h1 className="text-8xl font-bold">Hi. I'm Hassan</h1>
        <h2 className="text-5xl font-black">React.js Front-end Developer</h2>
        <p className="font-semibold text-base/loose pt-16 w-3/4 mx-auto">
          I'm dedicated to creating responsive interfaces. Enthusiastic about
          clean code and contributing to innovative open-source projects.
        </p>
      </div>
    </div>
  );
};

export default Banner;
